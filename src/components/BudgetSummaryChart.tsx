...
const BudgetSummaryChart = ({ selectedMonth, parentTransactions }) => {
  ...
  if (!categories.length) {
    return (
      <p style={{ textAlign: "center", marginTop: "20px", fontSize: "16px", fontWeight: "500" }}>
        No data available
      </p>
    );
  }
  ...
  const chartData = categories.map((category) => {
    ...
    const allocated = Math.round((data[selectedMonth]?.budget * category.percentage) / 100);
    ...
    <div style={{ width: "100%", height: "320px", padding: "10px" }}>
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={chartData} barGap={10}>
          ...
          <Bar dataKey="Allocated" stackId="a" fill={COLORS.Allocated} radius={[8, 8, 0, 0]} />
          <Bar dataKey="Spent" stackId="a" fill={COLORS.Spent} radius={[8, 8, 0, 0]} />
        </BarChart>
      </ResponsiveContainer>
    </div>
  );
